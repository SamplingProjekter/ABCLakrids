...
En anden problematik er bogstaver vi ikke kan kategorisere med sikkerhed samt bogstaver vi ikke har samplet. 
Som nævnt ovenfor antages derfor, at vi betragter med et alfabet med 27 bogstaver (uden ø og å, inklusiv w).
Det er også bogstaver som vi ikke umiddelbart kan se forskel på f.eks. Z'er og N'er. Løsningen på dette,er  at sortere disse som halv halv, det har dog den konsekvens, at vi muligvis introducere bias i vores stikprøve, da vi ikke med sikkerhed kan sige at der findes både z'er og N'er eller W'er og M'er.
...
## Sammenligning af interessevariable med alfabetet
...
<<<<<<< HEAD
Hvis vi ønsker at skrive et bestemt ord, kræver det ofte (i alt fald på dansk), at vi har en rimelig mængde af vokaler til rådighed. Lad derfor $y_v(i)$ betegne indikatorfunktionen for at individ $i$ er en vokal.
...
```{R}
id <- ifelse(rownames(Dat_new)%in% c("A","E","I","O","U","Y","Æ")
=======
